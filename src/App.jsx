    ...
    if(sec==="conv-cashout")return(<>
      ...
      {allTimePpl.filter(([,v])=>v.total>0).length>0?<div style={{display:"grid",gridTemplateColumns:`repeat(${Math.min(allTimePpl.filter(([,v])=>v.total>0).length,2)},1fr)`,gap:10}}>
        {allTimePpl.filter(([,v])=>v.total>0).map(([p,v])=>{const bal=v.total;return(<div key={p} style={{...cr,padding:"14px 16px",borderTop:`3px solid ${CO[p]||"#888"}`}}>
          ...
          <button onClick={()=>{const amt=parseFloat(prompt(`Clear ${p}'s convention balance?\nCurrent: ${FX(bal)}\n\nEnter amount to clear (or leave blank for all):`)||bal);if(!amt||isNaN(amt)||amt<=0||amt>bal)return;if(!confirm(`Clear ${FX(amt)} from ${p}'s convention balance?\nThis will clear the convention balance and credit ${FX(amt)} to ${p}'s Amex.`))return;const gid=`CONV-CLR-${Date.now()}`;const a=Math.round(amt*100)/100;const ne=[...entries,{id:Date.now(),c:"amex",d:TODAY,p,a,io:"XFER_OUT",r:`Convention balance cleared (settled outside)`,grp:gid,conv:true,t:new Date().toISOString()},{id:Date.now()+1,c:"amex",d:TODAY,p,a,io:"XFER_IN",r:`Convention settled → Amex`,grp:gid,t:new Date().toISOString()}];sE(ne);sv(ne);tw(`✓ Cleared ${FX(amt)} from ${p}'s convention → Amex`);}} style={{marginTop:6,padding:"6px 12px",borderRadius:6,border:"1px solid #52525b40",background:"rgba(82,82,91,.15)",color:"#a1a1aa",cursor:"pointer",fontSize:10,fontWeight:600,width:"100%"}}>Clear Balance (already settled)</button>
        </div>);})}
          ...
          {convItems.map((it,idx)=>{const isU=it.owner==="UNKNOWN";const itCh=it.ch||impCh;
            ...
            return(<div key={it.id}>
              {newOrd&&it.order&&<div style={{padding:"6px 14px",background:"rgba(249,115,22,.06)",borderTop:idx>0?"2px solid #f9731540":"none",marginTop:idx>0?4:0,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",borderLeft:"3px solid #f97316"}}>
                ...
                {it.staff&&<span style={{fontSize:8,fontWeight:600,padding:"1px 6px",borderRadius:4,border:"1px solid #a78bfa30",background:"rgba(167,139,250,.08)",color:"#a78bfa"}}>👤 {it.staff}</span>}
                {(()=>{const chs=[...new Set(grp.map(i=>i.ch||impCh))];return chs.map(ch=>{const chC=CC[ch]||"#71717a";return <span key={ch} style={{fontSize:8,fontWeight:700,padding:"1px 6px",borderRadius:4,border:`1px solid ${chC}40`,background:`${chC}10`,color:chC}}>→ {CL[ch]||ch}</span>;});})()}
