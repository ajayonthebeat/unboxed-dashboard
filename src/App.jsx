    ...
    if(sec==="conv-cashout")return(<>
      ...
      {allTimePpl.filter(([,v])=>v.total>0).length>0?<div style={{display:"grid",gridTemplateColumns:`repeat(${Math.min(allTimePpl.filter(([,v])=>v.total>0).length,2)},1fr)`,gap:10}}>
        {allTimePpl.filter(([,v])=>v.total>0).map(([p,v])=>{const bal=v.total;return(<div key={p} style={{...cr,padding:"14px 16px",borderTop:`3px solid ${CO[p]||"#888"}`}}>
          ...
          <button onClick={()=>{const amt=parseFloat(prompt(`Clear ${p}'s convention balance?\nCurrent: ${FX(bal)}\n\nEnter amount to clear (or leave blank for all):`)||bal);if(!amt||isNaN(amt)||amt<=0||amt>bal)return;if(!confirm(`Clear ${FX(amt)} from ${p}'s convention balance?\nThis just zeroes out the convention side — no accounts affected.`))return;const gid=`CONV-CLR-${Date.now()}`;const ne=[...entries,{id:Date.now(),c:"amex",d:TODAY,p,a:Math.round(amt*100)/100,io:"XFER_OUT",r:`Convention balance cleared (settled outside)`,grp:gid,conv:true,t:new Date().toISOString()}];sE(ne);sv(ne);tw(`✓ Cleared ${FX(amt)} from ${p}'s convention balance`);}} style={{marginTop:6,padding:"6px 12px",borderRadius:6,border:"1px solid #52525b40",background:"rgba(82,82,91,.15)",color:"#a1a1aa",cursor:"pointer",fontSize:10,fontWeight:600,width:"100%"}}>Clear Balance (already settled)</button>
        </div>);})}
