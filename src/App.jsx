...
{tab==="input"&&<>
  ...
  {sec==="manual"&&<div style={{...cr,padding:"20px"}}>
    ...
    {discMode&&<div style={{...cr,padding:"12px",marginBottom:10}}>
      ...
      <button onClick={()=>{const txt=document.getElementById("disc-paste").value;if(!txt.trim()){tw("⚠ Paste an order first");return;}const r=parseDiscord(txt);if(r.items.length===0){tw("⚠ No items found");return;}const flagged=flagDupes(r.items);const dupeCount=flagged.filter(i=>i.fl.includes("duplicate")).length;sII(prev=>{if(!prev)return flagged;const base=prev.length;return[...prev,...flagged.map((it,i)=>({...it,id:base+i}))];});sIC(r.channel);sIF("all");sImpSrc("discord");document.getElementById("disc-paste").value="";tw(dupeCount?`✓ Parsed ${r.items.length} items (⚠ ${dupeCount} already imported)`:`✓ Parsed ${r.items.length} items from Discord`);}} style={{padding:"6px 16px",borderRadius:6,border:"none",background:"#7c3aed",color:"#fafafa",cursor:"pointer",fontSize:11,fontWeight:700}}>Import</button>
    </div>}
    {/* === IMPORT PREVIEW === */}
    {impItems&&impStats&&<>
