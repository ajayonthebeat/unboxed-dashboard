...
class EB extends React.Component{constructor(p){super(p);this.state={error:null};}static getDerivedStateFromError(e){return{error:e};}render(){if(this.state.error)return(<div style={{padding:20,textAlign:"center"}}><div style={{color:"#ef4444",fontSize:12,marginBottom:8}}>Something went wrong</div><div style={{color:"#71717a",fontSize:10,marginBottom:12}}>{this.state.error?.message||"Unknown error"}</div><button onClick={()=>this.setState({error:null})} style={{padding:"6px 16px",borderRadius:6,border:"none",background:"#3f3f46",color:"#fafafa",cursor:"pointer",fontSize:11}}>Retry</button></div>);return this.props.children;}}
...
const CC={"shopify":"#96bf48","square":"#555d63","cash":"#22c55e","amex":"#3b82f6","venmo":"#008CFF","zelle":"#6D1ED4","unboxed":"#f97316"};
...
export default function App(){
  ...
  const handleFile=(e,type)=>{const file=e.target.files[0];if(!file)return;const reader=new FileReader();
    reader.onload=ev=>{const txt=ev.target.result;const r=type==="shopify"?parseShopifyCSV(txt):type==="unboxed"?parseUnboxedCSV(txt):parseSquareCSV(txt,pplInfo);sII(flagDupes(r.items));sIC(r.channel);sIF("all");sImpSrc(type);sSEC("manual");};reader.readAsText(file);e.target.value="";};
...
{tab==="sales"&&<>
  ...
  {sec==="goal"&&(()=>{const goal=100000;const startDate=new Date("2026-01-01");const today=new Date();const daysSoFar=Math.floor((today-startDate)/(86400000));
    ...
    const allProj=PP.filter(p=>(lastB[p]||0)>0).map(p=>{const t=lastB[p]||0;const avg=daysSoFar>0?t/daysSoFar:0;const exp=dailyTarget*daysSoFar;const rem=goal-t;const dn=avg>0?Math.ceil(rem/avg):9999;const ed=new Date(today);ed.setDate(ed.getDate()+dn);
    ...
    if(sec==="conv-cashout")return(<>
      ...
      {allTimePpl.filter(([,v])=>v.total>0).length>0?<div style={{display:"grid",gridTemplateColumns:`repeat(${Math.min(allTimePpl.filter(([,v])=>v.total>0).length,2)},1fr)`,gap:10}}>
        {allTimePpl.filter(([,v])=>v.total>0).map(([p,v])=>{const bal=v.total;return(<div key={p} style={{...cr,padding:"14px 16px",borderTop:`3px solid ${CO[p]||"#888"}`}}>
          ...
          <button onClick={()=>{if(!confirm(`Clear ${p}'s full convention balance of ${FX(bal)}?\nThis zeroes out all channels (cash, venmo, zelle, amex).`))return;const gid=`CONV-CLR-${Date.now()}`;let idx=0;const clrEntries=Object.entries(v.byCh).filter(([,a])=>a>0).map(([ch,a])=>({id:Date.now()+ ++idx,c:ch,d:TODAY,p,a:Math.round(a*100)/100,io:"XFER_OUT",r:`Convention ${CL[ch]||ch} cleared`,grp:gid,conv:true,t:new Date().toISOString()}));if(clrEntries.length===0)return;const ne=[...entries,...clrEntries];sE(ne);sv(ne);tw(`✓ Cleared ${FX(bal)} from ${p}'s convention balance`);}} style={{marginTop:6,padding:"6px 12px",borderRadius:6,border:"1px solid #52525b40",background:"rgba(82,82,91,.15)",color:"#a1a1aa",cursor:"pointer",fontSize:10,fontWeight:600,width:"100%"}}>Clear Balance (already settled)</button>
        </div>);})}
...
{tab==="input"&&<>
  ...
  {sec==="manual"&&<div style={{...cr,padding:"20px"}}>
    ...
    {discMode&&<div style={{...cr,padding:"12px",marginBottom:10}}>
      ...
      <button onClick={()=>{const txt=document.getElementById("disc-paste").value;if(!txt.trim()){tw("⚠ Paste an order first");return;}const r=parseDiscord(txt);if(r.items.length===0){tw("⚠ No items found");return;}const flagged=flagDupes(r.items);const dupeCount=flagged.filter(i=>i.fl.includes("duplicate")).length;if(dupeCount===flagged.length){tw("⚠ All items already imported");document.getElementById("disc-paste").value="";return;}const fresh=flagged.filter(i=>!i.fl.includes("duplicate"));sII(prev=>{if(!prev)return fresh;const pendingOrds=new Set(prev.map(i=>i.order).filter(Boolean));const newItems=fresh.filter(i=>!i.order||!pendingOrds.has(i.order));if(!newItems.length)return prev;const base=prev.length;return[...prev,...newItems.map((it,i)=>({...it,id:base+i}))];});sIC(r.channel);sIF("all");sImpSrc("discord");document.getElementById("disc-paste").value="";const skipped=flagged.length-fresh.length;tw(skipped?`✓ Parsed ${fresh.length} items (${skipped} duplicates skipped)`:`✓ Parsed ${fresh.length} items from Discord`);}} style={{padding:"6px 16px",borderRadius:6,border:"none",background:"#7c3aed",color:"#fafafa",cursor:"pointer",fontSize:11,fontWeight:700}}>Import</button>
    </div>}
    {/* === IMPORT PREVIEW === */}
    {impItems&&impStats&&<>
      ...
      <div style={{...cr,padding:0,overflow:"hidden"}}><div style={{maxHeight:380,overflowY:"auto"}}>
        {(()=>{const regItems=filtImp.filter(it=>it.saleType!=="convention");const ordGrp={};regItems.forEach(it=>{const o=it.order||`_${it.id}`;if(!ordGrp[o])ordGrp[o]=[];ordGrp[o].push(it);});
          return regItems.map((it,idx)=>{
            const isU=it.owner==="UNKNOWN";const isSplit=it.fl.includes("split_pay");
            const prevOrd=idx>0?regItems[idx-1].order:null;const newOrd=it.order!==prevOrd;
            ...
            return(<div key={it.id}>
              {newOrd&&it.order&&<div style={{padding:"6px 14px",background:it.saleType==="convention"?"rgba(249,115,22,.08)":"rgba(6,182,212,.04)",borderTop:idx>0?`2px solid ${it.saleType==="convention"?"#f9731540":"#52525b"}`:"none",marginTop:idx>0?4:0,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",borderLeft:it.saleType==="convention"?"3px solid #f97316":"none"}}>
                ...
                {(()=>{const ch=grp[0]?.ch||impCh;const chL=CL[ch]||ch;const chC=CC[ch]||"#71717a";const rcvrs=[...new Set(grp.filter(i=>i.rcvdBy).map(i=>i.rcvdBy))];return <span style={{fontSize:8,fontWeight:700,padding:"1px 6px",borderRadius:4,border:`1px solid ${chC}40`,background:`${chC}10`,color:chC}}>→ {chL}{(ch==="venmo"||ch==="zelle")&&rcvrs.length>0&&` (${rcvrs.map(r=>r+"'s acct").join(", ")})`}</span>;})()}
                ...
                <button onClick={()=>{sII(prev=>{if(!prev)return prev;const base=Math.max(...prev.map(i=>i.id))+1;const ref=grp[0]||it;return[...prev,{id:base,date:ref.date,name:"New Item",amt:0,owner:"UNKNOWN",fl:["unknown"],order:it.order,device:"",ch:ref.ch||impCh,staff:ref.staff||"",notes:"",saleType:ref.saleType||""}];});}} style={{marginLeft:"auto",padding:"2px 8px",borderRadius:4,border:"1px solid #06b6d440",background:"rgba(6,182,212,.08)",color:"#06b6d4",cursor:"pointer",fontSize:8,fontWeight:700,flexShrink:0}}>+ Person</button>
                <button onClick={()=>sDelConfId(sDelConfId===it.order?null:it.order)} style={{padding:"1px 6px",borderRadius:4,border:"1px solid #ef444430",background:delConfId===it.order?"rgba(239,68,68,.15)":"transparent",color:"#ef4444",cursor:"pointer",fontSize:9,fontWeight:600}}>×</button>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:isSplit?6:0}}>
            ...
            <input type="number" value={it.amt} onChange={e=>{const v=parseFloat(e.target.value);if(!isNaN(v)&&v>=0){editSplit(it.id,"amt",v);if(it.personAmt!=null&&it.personAmt>v)editSplit(it.id,"personAmt",v);}}} style={{...is,width:62,padding:"2px 6px",fontSize:12,fontWeight:700,color:"#22c55e",fontFamily:"monospace",borderRadius:5,textAlign:"right"}}/>
            ...
            <button onClick={()=>editSplit(it.id,"saleType","convention")} title="Move to Convention" style={{padding:"2px 6px",borderRadius:4,border:"1px solid #f9731630",background:"transparent",color:"#f97316",cursor:"pointer",fontSize:9,lineHeight:1,flexShrink:0,opacity:.6}}>🎪</button>
      ...
      {(()=>{const convItems=filtImp.filter(it=>it.saleType==="convention");if(!convItems.length)return null;
        const convTotal=convItems.reduce((s,i)=>s+i.amt,0);
        const ordGrp={};convItems.forEach(it=>{const o=it.order||`_${it.id}`;if(!ordGrp[o])ordGrp[o]=[];ordGrp[o].push(it);});
        const byChTotals={};convItems.forEach(it=>{const ch=it.ch||impCh;byChTotals[ch]=(byChTotals[ch]||0)+it.amt;});
        return(<div style={{...cr,padding:0,overflow:"hidden",marginTop:10,borderColor:"#f9731640"}}>
          <div style={{padding:"10px 14px",background:"rgba(249,115,22,.08)",borderBottom:"1px solid #f9731630"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{color:"#f97316",fontSize:13,fontWeight:800}}>🎪 Convention Sales</span>
              <div style={{display:"flex",alignItems:"center",gap:10}}><span style={{color:"#71717a",fontSize:9}}>{convItems.length} sale{convItems.length>1?"s":""} · {Object.keys(ordGrp).length} order{Object.keys(ordGrp).length>1?"s":""}</span><span style={{color:"#f97316",fontFamily:"monospace",fontSize:16,fontWeight:800}}>{FX(convTotal)}</span></div>
            </div>
            <div style={{display:"flex",gap:8,marginTop:6,flexWrap:"wrap"}}>{Object.entries(byChTotals).map(([ch,amt])=>{const chC=CC[ch]||"#71717a";return(<span key={ch} style={{fontSize:9,fontWeight:700,padding:"2px 8px",borderRadius:4,border:`1px solid ${chC}40`,background:`${chC}10`,color:chC}}>{CL[ch]||ch}: {FX(amt)}</span>);})}</div>
          </div>
          <div style={{maxHeight:380,overflowY:"auto"}}>
          {convItems.map((it,idx)=>{const isU=it.owner==="UNKNOWN";const itCh=it.ch||impCh;
            const prevOrd=idx>0?convItems[idx-1].order:null;const newOrd=it.order!==prevOrd;
            const grp=ordGrp[it.order||`_${it.id}`]||[];
            const ordTotal=grp.reduce((s,i)=>s+i.amt,0);
            return(<div key={it.id}>
              {newOrd&&it.order&&<div style={{padding:"6px 14px",background:"rgba(249,115,22,.06)",borderTop:idx>0?"2px solid #f9731540":"none",marginTop:idx>0?4:0,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",borderLeft:"3px solid #f97316"}}>
                <span style={{color:"#f97316",fontSize:12,fontWeight:800,fontFamily:"monospace"}}>{it.order}</span>
                <span style={{color:"#71717a",fontSize:9}}>{grp.length} item{grp.length>1?"s":""}</span>
                <span style={{color:"#fafafa",fontSize:11,fontWeight:700,fontFamily:"monospace"}}>{FX(ordTotal)}</span>
                {it.staff&&<span style={{fontSize:8,fontWeight:600,padding:"1px 6px",borderRadius:4,border:"1px solid #a78bfa30",background:"rgba(167,139,250,.08)",color:"#a78bfa"}}>👤 {it.staff}</span>}
                {(()=>{const chs=[...new Set(grp.map(i=>i.ch||impCh))];return chs.map(ch=>{const chC=CC[ch]||"#71717a";const rcvrs=[...new Set(grp.filter(i=>(i.ch||impCh)===ch&&i.rcvdBy).map(i=>i.rcvdBy))];return <span key={ch} style={{fontSize:8,fontWeight:700,padding:"1px 6px",borderRadius:4,border:`1px solid ${chC}40`,background:`${chC}10`,color:chC}}>→ {CL[ch]||ch}{(ch==="venmo"||ch==="zelle")&&rcvrs.length>0&&` (${rcvrs.map(r=>r+"'s acct").join(", ")})`}</span>;});})()}
                <button onClick={()=>{sII(prev=>{if(!prev)return prev;const base=Math.max(...prev.map(i=>i.id))+1;const ref=grp[0]||it;return[...prev,{id:base,date:ref.date,name:"New Item",amt:0,owner:"UNKNOWN",fl:["unknown"],order:it.order,device:"",ch:ref.ch||impCh,staff:ref.staff||"",notes:"",saleType:"convention"}];});}} style={{marginLeft:"auto",padding:"2px 8px",borderRadius:4,border:"1px solid #f9731640",background:"rgba(249,115,22,.08)",color:"#f97316",cursor:"pointer",fontSize:8,fontWeight:700,flexShrink:0}}>+ Person</button>
                <button onClick={()=>sDelConfId(sDelConfId===it.order?null:it.order)} style={{padding:"1px 6px",borderRadius:4,border:"1px solid #ef444430",background:delConfId===it.order?"rgba(239,68,68,.15)":"transparent",color:"#ef4444",cursor:"pointer",fontSize:9,fontWeight:600}}>×</button>
                {delConfId===it.order&&<button onClick={()=>removeImpOrder(it.order)} style={{padding:"2px 8px",borderRadius:4,border:"none",background:"#ef4444",color:"#fff",cursor:"pointer",fontSize:9,fontWeight:700}}>Delete Order?</button>}
              </div>}
              {newOrd&&!it.order&&idx>0&&<div style={{borderTop:"2px solid #f9731540",marginTop:4}}/>}
              <div style={{padding:"8px 14px",borderBottom:"1px solid rgba(63,63,70,.3)",borderLeft:"3px solid #f97316",background:isU?"rgba(239,68,68,.03)":"transparent"}}>
                <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                  {!(it.splits&&it.splits.length>0)&&<select value={it.owner} onChange={e=>editOwner(it.id,e.target.value)} style={{background:isU?"rgba(239,68,68,.15)":"rgba(63,63,70,.6)",border:`1px solid ${isU?"#ef444440":"#3f3f46"}`,borderRadius:6,padding:"3px 6px",color:isU?"#ef4444":CO[it.owner]||"#ccc",fontSize:10,fontWeight:600,cursor:"pointer",outline:"none",minWidth:75}}>
                    {isU&&<option value="UNKNOWN">⚠ ???</option>}<optgroup label="MAIN" style={{background:"#27272a"}}>{["AJAY","DEREK","SHARED","LJ"].map(p=>(<option key={p} value={p} style={{color:CO[p],background:"#27272a"}}>{p}</option>))}</optgroup><optgroup label="CONSIGNERS" style={{background:"#27272a"}}>{PP.filter(p=>!["AJAY","DEREK","SHARED","LJ"].includes(p)).sort().map(p=>(<option key={p} value={p} style={{color:CO[p],background:"#27272a"}}>{p}</option>))}</optgroup></select>}
                  <input type="number" value={it.amt} onChange={e=>{const v=parseFloat(e.target.value);if(!isNaN(v)&&v>=0)editSplit(it.id,"amt",v);}} style={{...is,width:62,padding:"2px 6px",fontSize:12,fontWeight:700,color:"#22c55e",fontFamily:"monospace",borderRadius:5,textAlign:"right"}}/>
                  <select value={itCh} onChange={e=>editSplit(it.id,"ch",e.target.value)} style={{background:`${CC[itCh]||"#555"}15`,border:`1px solid ${CC[itCh]||"#555"}40`,borderRadius:6,padding:"3px 6px",color:CC[itCh]||"#888",fontSize:9,fontWeight:700,cursor:"pointer",outline:"none"}}>
                    {[["cash","Cash"],["amex","Amex"],["venmo","Venmo"],["zelle","Zelle"]].map(([v,l])=>(<option key={v} value={v} style={{background:"#27272a",color:CC[v]}}>{l}</option>))}
                  </select>
                  {(itCh==="venmo"||itCh==="zelle")&&<div style={{display:"flex",alignItems:"center",gap:3}}>
                    <span style={{color:CC[itCh],fontSize:8,fontWeight:600}}>rcvd by:</span>
                    <select value={it.rcvdBy||""} onChange={e=>editSplit(it.id,"rcvdBy",e.target.value)} style={{background:"rgba(63,63,70,.6)",border:"1px solid #3f3f46",borderRadius:5,padding:"2px 5px",color:it.rcvdBy?CO[it.rcvdBy]||"#ccc":"#71717a",fontSize:9,fontWeight:600,cursor:"pointer",outline:"none"}}>
                      <option value="" style={{background:"#27272a",color:"#71717a"}}>Select...</option>
                      {["AJAY","DEREK","SHARED","LJ"].map(p=>(<option key={p} value={p} style={{background:"#27272a",color:CO[p]}}>{p}</option>))}
                    </select>
                  </div>}
                  <input value={it.note||""} onChange={e=>editSplit(it.id,"note",e.target.value)} placeholder="note..." style={{...is,flex:1,minWidth:50,maxWidth:120,padding:"2px 6px",fontSize:10,borderRadius:5,color:"#a1a1aa"}}/>
                  <div style={{minWidth:0,flex:1}}><div style={{color:"#d4d4d8",fontSize:10,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:120}}>{it.name}</div><div style={{color:"#a1a1aa",fontSize:9}}>{SD(it.date)}{it.staff&&` · 👤 ${it.staff}`}</div>{it.notes&&<div style={{color:"#71717a",fontSize:8,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:140}}>📝 {it.notes}</div>}</div>
                  {it.fl.map(f=>(<span key={f} style={{padding:"1px 6px",borderRadius:8,fontSize:8,fontWeight:600,background:`${FC[f]}15`,color:FC[f],border:`1px solid ${FC[f]}30`}}>{FL[f]}</span>))}
                  <button onClick={()=>editSplit(it.id,"saleType","")} title="Move to Regular" style={{padding:"2px 8px",borderRadius:4,border:"1px solid #3f3f46",background:"transparent",color:"#a1a1aa",cursor:"pointer",fontSize:8,fontWeight:600,flexShrink:0}}>↩ Regular</button>
                  {delConfId===it.id?<button onClick={()=>removeImpItem(it.id)} style={{padding:"2px 8px",borderRadius:4,border:"none",background:"#ef4444",color:"#fff",cursor:"pointer",fontSize:8,fontWeight:700,flexShrink:0}}>Delete?</button>:<button onClick={()=>sDelConfId(it.id)} style={{padding:"1px 5px",borderRadius:4,border:"1px solid #ef444430",background:"transparent",color:"#52525b",cursor:"pointer",fontSize:10,lineHeight:1,flexShrink:0}}>×</button>}
                </div>
              </div>
            </div>);})}
