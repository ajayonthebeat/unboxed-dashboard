...
export default function App(){
  ...
  useEffect(()=>{(async()=>{try{const r=await window.storage.get("ub-auth");if(r?.value)sAuthed(true);}catch(e){}
    try{const r=await window.storage.get("ub-vault");if(r?.value)sPwStore(JSON.parse(r.value));}catch(e){}})();},[]);
  const svVault=async v=>{try{await window.storage.set("ub-vault",JSON.stringify(v));}catch(e){}};
  const PIN="2026";
  const loginScreen=(<div style={{minHeight:"100vh",background:"#09090b",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
    <div style={{width:"100%",maxWidth:340,textAlign:"center"}}>
      ...
      <div style={{background:"rgba(24,24,27,.9)",borderRadius:12,padding:24,border:"1px solid #27272a"}}>
        ...
        <input type="password" value={loginPw} onChange={e=>sLoginPw(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&loginPw===PIN){sAuthed(true);window.storage.set("ub-auth","1").catch(()=>{});}}} placeholder="••••" style={{width:"100%",padding:"12px",borderRadius:8,border:"1px solid #3f3f46",background:"#18181b",color:"#fafafa",fontSize:18,textAlign:"center",letterSpacing:8,outline:"none",fontFamily:"monospace",marginBottom:12}}/>
        <button onClick={()=>{if(loginPw===PIN){sAuthed(true);window.storage.set("ub-auth","1").catch(()=>{});}else{sLoginPw("");alert("Wrong PIN");}}} style={{width:"100%",padding:"12px",borderRadius:8,border:"none",background:"#f59e0b",color:"#000",cursor:"pointer",fontSize:13,fontWeight:700,marginBottom:16}}>Unlock</button>
  ...
  const[poP,sPOP]=useState("AJAY");const[poCh,sPOCh]=useState("cash");const[poAmt,sPOAmt]=useState("");const[poNote,sPONote]=useState("");const[poDt,sPODt]=useState(TODAY);const[poType,sPOType]=useState("BUY");const[poFrom,sPOFrom]=useState(daysAgo(7));const[poSel,sPOSel]=useState(new Set());
  ...
  const balances=useMemo(()=>{const b={};Object.entries(S_B).forEach(([p,v])=>{b[p]={cash:v.cash,amex:v.amex,overall:v.overall};});
    entries.forEach(e=>{if(!b[e.p])b[e.p]={cash:0,amex:0,overall:0};
      if(e.io==="REFUND"){const amt=-e.a;if(e.c==="cash"||e.c==="square"){b[e.p].cash+=amt;}else if(e.c==="amex"||e.c==="trade"){b[e.p].amex+=amt;}else{b[e.p].cash+=amt;}b[e.p].overall+=amt;return;}
      const isIn=e.io==="IN"||e.io==="CONSIGNMENT"||e.io==="TRADE_IN"||e.io==="XFER_IN";const amt=isIn?e.a:-e.a;
      if(e.c==="cash"||e.c==="square"){b[e.p].cash+=amt;}else if(e.c==="amex"||e.c==="trade"){b[e.p].amex+=amt;}else{b[e.p].cash+=amt;}
      b[e.p].overall+=amt;});
    return b;},[entries]);
  ...
  useEffect(()=>{(async()=>{try{const r=await window.storage.get("ub-e3");if(r?.value){let p=JSON.parse(r.value);
    ...
    if(toAdd.length>0){p=[...p,...toAdd];try{await window.storage.set("ub-e3",JSON.stringify(p));}catch(e){}}
    ...
    if(migFixed){try{await window.storage.set("ub-e3",JSON.stringify(p));}catch(e){}}
    ...
    if(!p.find(e=>e.r==="Pending draw: AMEX split"&&e.p==="AJAY")){
      ...
      try{await window.storage.set("ub-e3",JSON.stringify(p));}catch(e){}
    }
    sE(p);
    ...
    try{const r2=await window.storage.get("ub-bank");if(r2?.value)sBankTxns(JSON.parse(r2.value));}catch(e){}
    // Load people info from local
    try{const r3=await window.storage.get("ub-ppl");if(r3?.value)sPplInfo(JSON.parse(r3.value));}catch(e){}
    })();},[]);
  const sv=useCallback(async ne=>{try{await window.storage.set("ub-e3",JSON.stringify(ne));}catch(e){}},[]);
  const svBank=useCallback(async nt=>{try{await window.storage.set("ub-bank",JSON.stringify(nt));}catch(e){}},[]);
  const svPpl=useCallback(async info=>{try{await window.storage.set("ub-ppl",JSON.stringify(info));}catch(e){}},[]);
  ...
  const submitCart=()=>{if(cart.length===0){tw("⚠ Cart empty",2000);return;}
    ...
    tw(`✓ Order: ${cart.length} items, ${FF(cart.reduce((s,c)=>s+c.total,0))} → ${Object.keys(Object.fromEntries(cart.map(c=>[c.p,1]))).join(", ")}`);sCart([]);};
  ...
  const applyImport=()=>{if(!impItems)return;const ne=[...entries];let cnt=0;let refCnt=0;
    ...
    for(const it of impItems){if(it.owner==="UNKNOWN")continue;
      if(it.fl.includes("refunded")){refCnt++;continue;}
      const isSpl=it.fl.includes("split_pay");
      const shopA=isSpl?(it.shopAmt!=null?it.shopAmt:it.amt):it.amt;
      const cashA=isSpl?(it.cashAmt||0):0;
      const tradeA=isSpl?(it.tradeAmt||0):0;
      const label=it.order?`${it.order} · ${it.name||impCh}`:`${impCh} import`;
      ...
      const actualCh=ch==="shopify"?(pmUp.includes("CASH")?"cash":pmUp.includes("AMEX")||pmUp.includes("AMERICAN EXPRESS")?"amex":"shopify"):ch;
      ...
      const hasProcFee=(actualCh==="shopify"||actualCh==="square");
      for(const pt of portions){
        if(pt.amt<=0)continue;
        const ptFee=(pt.c==="shopify"||pt.c==="square")?procFee:0;
        if(isOwner){
    ...
    sDD({...nd});sCD({...nc});tw(`✓ Imported ${ne.length-entries.length} entries${refCnt>0?` (${refCnt} refunded skipped)`:""}`);sII(null);};
  ...
  const impStats=useMemo(()=>{if(!impItems)return null;const fl=impItems.filter(i=>i.fl.length>0);const unk=impItems.filter(i=>i.owner==="UNKNOWN");const spl=impItems.filter(i=>i.fl.includes("split_pay"));const ref=impItems.filter(i=>i.fl.includes("refunded"));
  ...
  return(<div style={{minHeight:"100vh",background:"#18181b",padding:"12px 8px",fontFamily:"'Segoe UI',system-ui,sans-serif",color:"#fafafa",fontSize:13}}>
...
{tab==="input"&&<>
      ...
      <div style={{...cr,padding:0,overflow:"hidden"}}><div style={{maxHeight:380,overflowY:"auto"}}>
        {(()=>{const ordGrp={};filtImp.forEach(it=>{const o=it.order||`_${it.id}`;if(!ordGrp[o])ordGrp[o]=[];ordGrp[o].push(it);});
          ...
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:isSplit?6:0}}>
            ...
            <select value={it.owner} onChange={e=>editOwner(it.id,e.target.value)} style={{background:isU?"rgba(239,68,68,.15)":"rgba(63,63,70,.6)",border:`1px solid ${isU?"#ef444440":"#3f3f46"}`,borderRadius:6,padding:"3px 6px",color:isU?"#ef4444":CO[it.owner]||"#ccc",fontSize:10,fontWeight:600,cursor:"pointer",outline:"none",minWidth:75}}>
            ...
            <span style={{color:"#22c55e",fontFamily:"monospace",fontSize:12,fontWeight:700,minWidth:50}}>{FX(it.amt)}</span>
            <div style={{flex:1,minWidth:0}}><div style={{color:"#d4d4d8",fontSize:10,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{it.name}</div><div style={{color:"#a1a1aa",fontSize:9}}>{SD(it.date)}{it.pm&&` · ${it.pm}`}</div></div>
  ...
  {sec==="money"&&<div style={{...cr,padding:"20px"}}>
    ...
    {poType==="BUY"&&<>
    <div style={{color:"#71717a",fontSize:10,fontWeight:600,letterSpacing:1,marginBottom:10}}>💸 BUY / PULL OUT / EXPENSE</div>
    <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}>
      <div style={{minWidth:90}}><div style={{color:"#a1a1aa",fontSize:9,marginBottom:3}}>PERSON</div>
        <select value={poP} onChange={e=>sPOP(e.target.value)} style={{...is,width:"100%",padding:"6px 8px",fontSize:12,color:CO[poP]||"#fff"}}><optgroup label="MAIN" style={{background:"#27272a"}}>{["AJAY","DEREK","SHARED","LJ"].map(p=>(<option key={p} value={p} style={{color:CO[p],background:"#27272a"}}>{p}</option>))}</optgroup></select></div>
    ...
    {(()=>{const a=parseFloat(poAmt)||0;const bal=balances[poP]||{cash:0,amex:0,overall:0};const chBal=poCh==="cash"?bal.cash:poCh==="amex"?bal.amex:bal.overall;
    ...
    <button onClick={()=>{const a=parseFloat(poAmt)||0;if(a<=0){tw("⚠ Enter amount",2000);return;}
      ...
      const e={id:Date.now()+Math.random(),c:poCh,d:poDt,p:poP,a:Math.round(a*100)/100,io:"OUT",r:`BUY: ${desc}`,t:new Date().toISOString()};
      const ne=[...entries,e];sE(ne);sv(ne);
      tw(`✓ -${FX(a)} from ${poP} (${poCh})`);sPOAmt("");sPONote("");
    }} style={{padding:"10px 24px",borderRadius:8,border:"none",background:"#ef4444",color:"#fafafa",cursor:"pointer",fontSize:13,fontWeight:700,width:"100%"}}>💸 Record Buy / Pull Out</button>
    </>}
    ...
    {poType==="CONSIGN_PAY"&&(()=>{
      ...
      <div style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap",alignItems:"center"}}>
        ...
        <button onClick={()=>{
          ...
          const isIOS=/iPhone|iPad|iPod/.test(navigator.userAgent);const sep=isIOS?"&":"?";
          const texts=[];
          selected.forEach((c,idx)=>{
            ...
            if(pplInfo[c.p]?.phone){texts.push({phone:pplInfo[c.p].phone,amt:totalAmex,name:c.p});}
          });
          sE(ne);sv(ne);sPOSel(new Set());
          tw(`✓ Paid out ${selected.length} consigners: ${FX(selTotal)}`);
          if(texts.length>0){
            texts.forEach((t,i)=>{
              const msg=`UNBOXEDTCG: YOUR PAYOUT OF $${t.amt.toFixed(2)} HAS JUST BEEN SENT OUT, SHOULD ARRIVE IN YOUR BANK IN THE NEXT 48 HOURS. PAYOUT FROM ${fmtFrom} to ${fmtTo}`;
              setTimeout(()=>window.open(`sms:${t.phone}${sep}body=${encodeURIComponent(msg)}`),400*(i+1));
            });
          }
        }} style={{padding:"5px 12px",borderRadius:5,border:"none",background:"#ef4444",color:"#fafafa",cursor:"pointer",fontSize:9,fontWeight:700}}>💰📱 Pay + Text Selected ({selected.length})</button>
      </div>);})()}
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {consigners.map(c=>(<div key={c.p} style={{...cr,padding:"14px 16px",border:`1px solid ${poSel.has(c.p)?CO[c.p]+"60":CO[c.p]+"20"}`,background:poSel.has(c.p)?"rgba(245,158,11,.03)":"transparent"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
              <div style={{display:"flex",alignItems:"center",gap:6}}>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              {Math.abs(c.cash)>0.01&&<button onClick={()=>{
                const amt=Math.round(Math.abs(c.cash)*100)/100;
                const ne=[...entries,
                  {id:Date.now()+Math.random(),c:"cash",d:poDt,p:c.p,a:amt,io:c.cash>0?"XFER_OUT":"XFER_IN",r:"Balance: cash→amex",t:new Date().toISOString()},
                  {id:Date.now()+Math.random()+.001,c:"amex",d:poDt,p:c.p,a:amt,io:c.cash>0?"XFER_IN":"XFER_OUT",r:"Balance: cash→amex",t:new Date().toISOString()}
                ];sE(ne);sv(ne);tw(`✓ ${c.p}: moved ${FX(amt)} cash→amex`);
              }} style={{padding:"5px 10px",borderRadius:5,border:"1px solid #3b82f640",background:"rgba(59,130,246,.1)",color:"#3b82f6",cursor:"pointer",fontSize:9,fontWeight:600}}>Balance Cash→Amex</button>}
              <button onClick={()=>{
                ...
                const totalAmex=Math.round(Math.abs(c.overall)*100)/100;
                ne.push({id:Date.now()+Math.random()+.002,c:"amex",d:poDt,p:c.p,a:totalAmex,io:"PYOUT",r:`PAYOUT: Weekly consigner payout`,grp:gid,pyout:true,t:new Date().toISOString()});
                sE(ne);sv(ne);tw(`✓ Paid out ${c.p}: ${FX(totalAmex)}`);
                if(pplInfo[c.p]?.phone){
                  const fmtFrom=SD(poFrom);const fmtTo=SD(poDt);
                  const msg=`UNBOXEDTCG: YOUR PAYOUT OF $${totalAmex.toFixed(2)} HAS JUST BEEN SENT OUT, SHOULD ARRIVE IN YOUR BANK IN THE NEXT 48 HOURS. PAYOUT FROM ${fmtFrom} to ${fmtTo}`;
                  const isIOS=/iPhone|iPad|iPod/.test(navigator.userAgent);
                  const sep=isIOS?"&":"?";
                  setTimeout(()=>window.open(`sms:${pplInfo[c.p].phone}${sep}body=${encodeURIComponent(msg)}`),300);
                }
              }} style={{padding:"5px 14px",borderRadius:5,border:"none",background:"#ef4444",color:"#fafafa",cursor:"pointer",fontSize:9,fontWeight:700}}>{pplInfo[c.p]?.phone?"💰📱 Pay + Text":"💰 Pay Out"} {FX(c.overall)}</button>
            </div>
          </div>))}
      </div></>}
      </>);
      ...
      <div style={{display:"flex",flexDirection:"column",gap:2}}>
        {Object.entries(byDate).sort(([a],[b])=>b.localeCompare(a)).map(([d,es])=>(<div key={d}>
          ...
          {es.map(e=>(<div key={e.id} style={{display:"flex",alignItems:"center",gap:6,padding:"6px 12px 6px 24px",borderBottom:"1px solid rgba(63,63,70,.15)"}}>
            ...
            {pplInfo[e.p]?.phone&&e.pyout&&<button onClick={()=>{
              const isIOS=/iPhone|iPad|iPod/.test(navigator.userAgent);const sep=isIOS?"&":"?";
              const msg=`UNBOXEDTCG: YOUR PAYOUT OF $${e.a.toFixed(2)} HAS JUST BEEN SENT OUT, SHOULD ARRIVE IN YOUR BANK IN THE NEXT 48 HOURS. PAYOUT FROM ${SD(poFrom)} to ${SD(e.d)}`;
              window.open(`sms:${pplInfo[e.p].phone}${sep}body=${encodeURIComponent(msg)}`);
  ...
  {sec==="bank"&&(()=>{
    ...
    const addBankTxn=()=>{const a=parseFloat(bkAmt);if(!a||a<=0){tw("⚠ Enter amount");return;}
      ...
      sBankTxns(nt);svBank(nt);sBkAmt("");sBkNote("");tw(`✓ ${bkType==="deposit"?"Deposited":"Withdrew"} ${FF(a)} → ${bkAcct}`);};
    ...
    return(<div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:12}}>
        {accts.map(a=>(<div key={a} onClick={()=>sBkAcct(a)} style={{...cr,padding:"14px",cursor:"pointer",borderLeft:`3px solid ${a==="AMEX"?"#06b6d4":a==="CASH"?"#22c55e":"#3b82f6"}`,background:bkAcct===a?"rgba(63,63,70,.7)":"rgba(63,63,70,.3)"}}>
          ...
          <div style={{color:bankBals[a]>=0?"#10b981":"#ef4444",fontSize:22,fontWeight:800,fontFamily:"monospace"}}>{FF(bankBals[a])}</div>
      ...
      <div style={{...cr,padding:"16px",marginBottom:12}}>
        ...
        {bkType==="deposit"&&(()=>{
          ...
          return(<div style={{background:"rgba(34,197,94,.05)",border:"1px solid #22c55e20",borderRadius:8,padding:"10px 12px",marginBottom:10}}>
            ...
            {bkAmt&&parseFloat(bkAmt)>0&&<div style={{marginTop:6,textAlign:"center",color:"#22c55e",fontSize:14,fontWeight:800,fontFamily:"monospace"}}>{FF(parseFloat(bkAmt))}</div>}
          </div>);
        })()}
      ...
      {acctTxns.length>0&&<div style={{...cr,overflow:"hidden",padding:0}}>
        ...
        <div style={{maxHeight:300,overflowY:"auto"}}>
          {acctTxns.map(t=>(<div key={t.id} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 14px",borderBottom:"1px solid rgba(63,63,70,.3)"}}>
            ...
            <span style={{color:t.type==="deposit"?"#10b981":"#ef4444",fontFamily:"monospace",fontSize:11,fontWeight:700,minWidth:55}}>{t.type==="deposit"?"+":"-"}{FF(t.amt)}</span>
  ...
  {sec==="log"&&(()=>{
    ...
    const filtered=entries.filter(e=>{
      if(logCh!=="all"&&e.c!==logCh)return false;
      ...
      <div style={{maxHeight:400,overflowY:"auto"}}>
        {(()=>{
          ...
          rev.forEach(e=>{
            ...
            } else if((e.c==="shopify"||e.c==="square")&&e.ord){
          ...
          return groups.map((g,gi)=>{
            if(g.type==="single"){
              ...
              return(<div key={e.id} onClick={logSelMode?()=>sLogSel(s=>{const n=new Set(s);n.has(e.id)?n.delete(e.id):n.add(e.id);return n;}):undefined} style={{display:"flex",alignItems:"center",gap:6,padding:"7px 14px",borderBottom:"1px solid rgba(63,63,70,.3)",background:sel?"rgba(239,68,68,.08)":"transparent",cursor:logSelMode?"pointer":"default",flexWrap:"wrap"}}>
                ...
                <span style={{color:CC[e.c],fontSize:8,fontWeight:700}}>{CL[e.c]}</span>
            ...
            const channels=g.type==="import"?[...new Set(g.entries.map(e=>e.c))]:[];
            return(<div key={g.key}>
              ...
              {isExp&&<div style={{borderBottom:"1px solid rgba(63,63,70,.3)"}}>
                ...
                return(<div key={e.id} onClick={logSelMode?ev=>{ev.stopPropagation();sLogSel(s=>{const n=new Set(s);n.has(e.id)?n.delete(e.id):n.add(e.id);return n;});}:undefined} style={{display:"flex",alignItems:"center",gap:5,padding:"5px 14px 5px 30px",borderBottom:"1px solid rgba(63,63,70,.15)",background:sel?"rgba(239,68,68,.05)":"transparent",cursor:logSelMode?"pointer":"default",fontSize:9,flexWrap:"wrap"}}>
                  ...
                  {g.type==="import"&&<span style={{color:CC[e.c]||"#71717a",fontSize:7,fontWeight:700,minWidth:28}}>{CL[e.c]}</span>}
          ...
          {members.map(p=>{const t=totals[p]||0;return(<div key={p} onClick={()=>sSelPerson(p)} style={{...cr,cursor:"pointer",borderLeft:`3px solid ${CO[p]}`}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:6}}>
              ...
              <div style={{display:"flex",gap:4,alignItems:"center"}}>
                {pplInfo[p]?.phone&&<span style={{color:"#52525b",fontSize:8}}>📱</span>}
                <span style={{color:"#52525b",fontSize:9}}>→</span>
              </div>
            </div>
          </div>);})}
  ...
  {selPerson&&(()=>{
    ...
    return(<div>
      ...
      <div style={{...cr,padding:"18px"}}>
        ...
        {info.phone&&<div style={{marginTop:12,display:"flex",gap:6}}>
          ...
          <button onClick={()=>window.open(`sms:${info.phone}`)} style={{padding:"6px 12px",borderRadius:5,border:"1px solid #3b82f640",background:"rgba(59,130,246,.1)",color:"#3b82f6",cursor:"pointer",fontSize:9,fontWeight:600}}>💬 Text</button>
        </div>}
      </div>
    </div>);
  })()}
      ...
      <div style={{color:"#222",fontSize:10,marginTop:28,textAlign:"center"}}>UNBOXED TCG · Arden Fair Mall<br/><button onClick={async()=>{try{await window.storage.delete("ub-auth");}catch(e){}sAuthed(false);sLoginPw("");}} style={{background:"transparent",border:"none",color:"#333",cursor:"pointer",fontSize:8,marginTop:4}}>🔒 Lock</button></div>
    </div>
  </div>);
}
